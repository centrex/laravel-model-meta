...
`centrex/laravel-model-meta` — Polymorphic metadata storage for any Eloquent model.

Namespace: `Centrex\Meta\`  
Service Provider: `MetaServiceProvider`  
Facade: `Facades/Meta`

## Commands

Run from inside this directory (`cd laravel-model-meta`):
...
- Attach arbitrary metadata (key-value pairs) to any Eloquent model polymorphically
- Similar to `laravel-model-data` but focused on metadata semantics
- Single `meta` table stores `metable_type`, `metable_id`, `key`, `value`
